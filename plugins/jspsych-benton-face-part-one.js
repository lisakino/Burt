/* Benton Facial Recognition Task Part 1
 * by Lisa Kinoshita (2017)
 * Template by Cool Jon Corp., 2017
 */
 
jsPsych.plugins["benton-face-part-one"] = (function() {
  var plugin = {};
  jsPsych.pluginAPI.registerPreload('benton-face-part-one', 'stimuli', 'image');
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var j = 0; j < trial.stimuli.length; j++) {
      default_timing_array.push(30000);
    }
    
    //30 seconds to respond each trial
    trial.timing_response = 30000;
    
    trial.is_html = true;
    trial.prompt = "Select one image";
    trial.responses = [];
 ...
 // function to end trial when it is time
    var end_trial = function() {
     ...
     //check number of responses
      if (trial.responses.length == 1) {
        console.log("One response=CHECK");
    ...
    function showStimuli() {
      ...
      display_element.append($('<div>', {
        html:'<div position = relative>'+
                '<div class = col-sm-4 col-sm-4 column1>'+
                ...
                '</div>'+
                '<div class = col-sm-4 col-sm-4 column2>'+
              ...
              '</div>'+
              '<div class = col-sm-4 col-sm-4 column3>'+
      ...
      $('.option').bind('click', function(){
        ...
        if (trial.responses.indexOf(option_selected) == -1 && trial.responses.length < 3){
          ...
          if (trial.responses.length === 1){
            ...
            console.log("clicked");
            after_response(option_selected);   
            });
          }
        } else {
          after_response(option_selected); 
    ...
    var after_response = function(info) {
      ...
      for (var i = 0; i < trial.stimuli.length; i++) {
        ...
        for (var j = 0; j < trial.stimuli.length; j++) {
          keycode = (typeof trial.stimuli[i] == 'string') ? jsPsych.pluginAPI.convertKeyCharacterToKeyCode(trial.stimuli[i]) : trial.stimuli[i];
          if (info.key == keycode) {
          console.log("for loop converting final answer");
            whichResponse = i;
            break;
          }
       }
